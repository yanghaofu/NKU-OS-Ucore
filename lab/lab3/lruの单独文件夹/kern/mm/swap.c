...
swap_in(struct mm_struct *mm, uintptr_t addr, struct Page **ptr_result)
{
    ...
    //list_entry_t* le = &mm->map_count;
    //while ((le = list_next(le)) != &mm->map_count) {
    //    struct vma_struct* mmap = le2vma(le, list_link);
    //    if (addr < mmap->vm_end && addr >= mmap->vm_start)
    //    {
    //        *ptr_result = pa2page(addr);
...
check_swap(void)
{
    //backup mem env
     ...
     cprintf("swap_check_ok!fuck you fuck you!!!!\n\n");
