...
int page_insert(pde_t *pgdir, struct Page *page, uintptr_t la, uint32_t perm) {
    ...
    //现在应该意思是，我的这个程序被分配到了当前的page，然后我要把它实际包括的物理地址映射一下，变成我当前程序看到的（以为的虚拟地址）。
    //比如我现在以为这个地方地址是0x008000（实际上它是虚拟的），但是分到的page是一个甚么够吧地方，然后就要就要把它映射到这个虚拟地址。
    pte_t *ptep = get_pte(pgdir, la, 1);
    //拿到一个页表项
    if (ptep == NULL) {
        return -E_NO_MEM;
    ...
    page_ref_inc(page);
    //指向这个page的映射加一。
    //一个物理页面（page）通常对应于一个物理内存页面框（frame），而多个虚拟页面可以映射到同一个物理页面上。
    if (*ptep & PTE_V) {
        struct Page *p = pte2page(*ptep);
        if (p == page) {
            page_ref_dec(page);
            //如果这个虚拟地址本来就是拿这个page来硬射的，那么就当无事发生。
        } else {
            page_remove_pte(pgdir, la, ptep);
            //如果当前虚拟地址被别的page硬射了，就把当前地址从这个page的硬射表上卸下来。
        }
    }
