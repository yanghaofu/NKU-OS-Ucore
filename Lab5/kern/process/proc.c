...
alloc_proc(void) {
     ...
     */
        ...
        proc->wait_state = 0; //表示进程等待状态
        proc->cptr = proc->optr = proc->yptr = NULL;//子、父、兄
...
set_links(struct proc_struct *proc) {
    list_add(&proc_list, &(proc->list_link));//添加到全局进程链表
    proc->yptr = NULL;
    if ((proc->optr = proc->parent->cptr) != NULL) {
        /*
        如果父进程已经有子进程（proc->optr不为NULL），
        则将当前进程设置为父进程的子进程的兄弟进程（optr->yptr）。
        这样，建立了当前进程与其兄弟进程之间的关联。
        */
        proc->optr->yptr = proc;
    }
